...
В *ECC* используется точно такая же кривая, только рассматриваемая над некоторым конечным полем ${F} _{p}=\mathbb{Z} / \mathbb{Z}_p = \{0, 1, ..., p - 1\}, где $ $p$ - простое число. То есть функция приобретает вид $y^2\ mod\ p = x^2 + ax + b \ (mod\ p)$.
...
​В результате, каждый Вася при желании сможет взять эту троицу, функцию вида `validate_signature(public_key, signature, text)` и проверить, действительно ли владелец приватного ключа подписывал это сообщение или нет. А если внутри сети все знают, что `public\_key` принадлежит Алисе, то можно понять, отправила эти деньги она или же кто-то пытается сделать это от ее имени.
